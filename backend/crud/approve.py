...
def approve_activity(db: Session, activity_id: int):
    activity = get_activity_by_id(db, activity_id)
    if activity.state != AcceptState.REQUESTED.value:
...
def super_approve_activity(db: Session, activity_id: int):
    activity = get_activity_by_id(db, activity_id)
...
def reject_activity(db: Session, activity_id: int):
    activity = get_activity_by_id(db, activity_id)
...
def create_note_for_activity(db: Session, activity_id: int, note: NoteCreate):
    activity = get_activity_by_id(db, activity_id)
    db_note = Notes(**note.dict(), activity_id=activity_id)
    db.add(db_note)
    db.commit()
    db.refresh(db_note)
    return db_note
